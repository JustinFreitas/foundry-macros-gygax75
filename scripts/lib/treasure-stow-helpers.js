/*
 * Shared helpers for the treasure-stow / consolidate macros.
 *
 * Foundry macros cannot `import`/`require` at runtime, so the relevant block of
 * this file is INLINED into the macros between the BEGIN/END SHARED marker
 * comments found below this header.
 *
 * The spec `shared-region-sync.spec.js` asserts that the inlined copies match
 * this canonical source byte-for-byte, so drift is caught by `npm test`.
 *
 * Only edit the shared logic HERE, then run `node scripts/lib/sync-shared.js`
 * to push it into the macros (or copy the region by hand).
 */
