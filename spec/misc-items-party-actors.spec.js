...
describe('misc-items-party-actors', () => {
  let scriptContent;
  ...
  beforeAll(() => {
    scriptContent = fs.readFileSync(path.resolve(__dirname, '../scripts/misc-items-party-actors.js'), 'utf8');
  });
  ...
  it('should create a chat message with "No unallowed Misc items" if there are no party actors', async () => {
    const script = `(async () => {${scriptContent}})()`;
    await eval(script);
    expect(global.ChatMessage.create).toHaveBeenCalledWith({
      content: '<h2>Misc Items Report</h2><br/>No unallowed Misc items in any party actor.',
    });
  });

  it('should create a chat message with "No unallowed Misc items" if there are no misc items in party actors', async () => {
    const actor = {
      type: 'character',
  ...
  it('should create a chat message with a report of misc items from party actors', async () => {
    const partyActor = {
      type: 'character',
      flags: { ose: { party: true } },
      name: 'Party Actor',
    ...
    const nonPartyActor = {
        ...
        name: 'Non-Party Actor',
        system: { details: { class: 'Fighter' } },
        items: [
          { type: 'item', name: 'Misc Item 3', system: { containerId: null } },
        ],
      };
    global.game.actors = [partyActor, nonPartyActor];
    const script = `(async () => {${scriptContent}})()`;
    await eval(script);
    expect(global.console.log).toHaveBeenCalledWith('<b>Party Actor:</b> Misc Item 1');
    expect(global.console.log).toHaveBeenCalledWith('<b>Party Actor:</b> Misc Item 2');
    expect(global.console.log).not.toHaveBeenCalledWith('<b>Non-Party Actor:</b> Misc Item 3');
    expect(global.ChatMessage.create).toHaveBeenCalledWith({
      content: '<h2>Misc Items Report</h2><br/><b>Party Actor:</b> Misc Item 1<br/><br/><b>Party Actor:</b> Misc Item 2',
    });
  });
});